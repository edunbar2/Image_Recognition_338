...
root.title("Common Animal Classifier")
...
def open_image(initialdir='/'):
    ...
    dialog_var.set("Select Your Image and Press Load")
...
def test_image():
    ...
    dialog_var.set("Image, Classified.")
...
#######   IMAGE input
btn_img_fopen = Button(top_frame, text='Select Image',  command =  lambda: open_image(img_entry.get()), bg="black", fg="white" )
...
btn_img_confirm = Button(top_frame, text='Load',  command = load_image , bg="black", fg="white" )
btn_img_confirm.grid(row=7, column=4)


""" middle Frame  """
ml = Label(middle_frame, font=("Airial", 10),bg="gray", fg="white", text="Browse Image Show Below").grid(row=1, column=1)
...
# Test Image butttom
btn_test = Button(bottom_frame, text='Classify',  command = test_image , bg="green", fg="white" )
btn_test.pack()


test_result_var = StringVar()
test_result_var.set("Output shown here")
test_result_label = Label(bottom_frame,font=("Airial", 20), height=3, textvariable=test_result_var, bg="white", fg="black").pack()
...
dialog_var.set("Select Your Image to be Classified.")


# Label frame
labelframe1 = LabelFrame(notification_frame, text="User Log", bg="yellow")
labelframe1.pack()

toplabel = Label(labelframe1,font=("Airial", 15), height=2, textvariable=dialog_var, fg="red", bg="white")
toplabel.pack()
